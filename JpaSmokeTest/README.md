...
`../CSharpEFTestProject/CSharpEFTestProject/Profiles/`. That directory's
`.gitignore` excludes generated profile output, but on this branch the three
files below are committed anyway (force-added) so the harness is reproducible
without a CIMTool install. Regenerate them as described here if the builders
change:
...
| `SampleProfile.xml` | CIMTool's XML profile serialisation | Input for the builders; kept so the XSLTs can be re-run outside the GUI |
...
1. Make sure your CIMTool instance is running the **latest** builders: copy
   ...
   (alongside `builders.json`, the registry that names them), then restart
   CIMTool.
...
3. In the profile's **Stylesheets/Builders** settings for
   `Profiles/SampleProfile.owl`, enable the `jpa-rdfs` and `sql-rdfs-ansi92`
   builders (both are registered in `builders.json`), and export the profile's
   XML serialisation.
4. Build the project — the files above appear next to `SampleProfile.owl`.

### Headless regeneration — not currently possible

`regen-fixtures.sh` runs the two builder XSLTs over `SampleProfile.xml` with
Saxon and writes the result to a scratch directory for inspection. It cannot
replace the fixtures: the XSLTs emit CIMTool's Indent-namespace XML, and the
step that renders that to plain text lives inside CIMTool with no standalone
entry point in this repository. The script therefore never overwrites the
committed fixtures. Regenerating them needs the CIMTool GUI.
...
The fixture location defaults to `../CSharpEFTestProject/CSharpEFTestProject/Profiles`
relative to this directory; override it with `-Dcimtool.profiles.dir=<dir>`.
...
|---|---|---|---|
| Compound cleanup on owner delete | Reverse `ON DELETE CASCADE` constraints | Forward `@ManyToOne(cascade=REMOVE)` from the owner | Same intent, different mechanism. Note: under immediate FK checking the SQL mechanism cannot execute — the forward owner FK blocks both compound inserts and the owner-delete cascade (sections 14/15 `DDL FINDING`s); deferred constraints do not help either, because a compound type reachable from two owner columns carries one reverse FK per slot and no row can satisfy all of them at once (verified on H2 and PostgreSQL 14.10) |
